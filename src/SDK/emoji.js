...
(function () {
    for (var i = 1; i <= 48; i++) {
        pinupList2['ajmd'].push('ajmd0' + (i >= 10 ? i : '0' + i) + '.png')
    }
    ;
    for (var i = 1; i <= 40; i++) {
        pinupList2['xxy'].push('xxy0' + (i >= 10 ? i : '0' + i) + '.png')
    }
    ;
    for (var i = 1; i <= 20; i++) {
        pinupList2['lt'].push('lt0' + (i >= 10 ? i : '0' + i) + '.png')
    }
    ;
})();
export let pinupList = pinupList2;
/**
...
export function buildEmoji(text) {
    ...
    for (let j = 0, len = matches.length; j < len; ++j) {
        if (emoji[matches[j]]) {
            text = text.replace(matches[j], '<img class="emoji" src="' + imageRoot + '/emoji/' + emoji[matches[j]].file + '" />');
        }
    }
    return text;
}

export function CEmojiEngine(emNode, emConfig) {
    emConfig = emConfig || {};
...
CEmojiEngine.prototype.__initXGui = function (emNode, emConfig) {
    ...
    this._lWidth = parseInt((this._width - 25) / 12, 10) - 20;		//小的缩略图
    this._hWidth = parseInt((this._width - 25) / 5, 10) - 20;			//大的缩略图
...
CEmojiEngine.prototype.__renderChangeCol = function () {
    for (var i in this._emojiList) {
        ...
        for (var key in emojiList) {
            ...
            span.id = 'chn-emoji-' + i;
            let img = new Image();
            img.src = getImagePath() + emojiList[key]['file'];
    ...
    for (var k in this._pinupList) {
        var pinupList = this._pinupList[k];
        let span = document.createElement('span');
        span.id = 'chn-pinup-' + k;
        let img = new Image();
        img.src = getImagePath() + pinupList[0];
        span.appendChild(img);
        this._changeColumn.appendChild(span);
    }
}
...
CEmojiEngine.prototype.__renderPictureCol = function () {
    ...
    if (isEmoji) {		//表情包
        ...
        for (let key = 0; key < pinupList.length; key++) {
            let span = document.createElement('span');
    ...
    //perfect scrollbar problem
    // $(this._pictureColumn).parent().scrollTop(0);
}
